  ...
  if (def.questions) {
    ...
    inner = def.questions.map((q, qi) => {
      ...
      return `<div class="q-item" data-q="${q.id}">
        <div class="q-section-title"><h3>${esc(q.label || `Topic ${qi + 1}`)}<span class="req">*</span></h3>
        <span class="req-note">Question ${qi + 1} of ${def.questions.length}</span></div>
    ...
    <div class="body-pad">
      ...
      ${a.options.map((opt, i) => {
        ...
        return `<button class="opt-choice ${sel === i ? 'sel' : ''}" data-i="${i}" style="animation-delay:${i * .06}s">
          <span class="oc-radio"></span>
          <span class="oc-icon-chip" style="--chip:${kind.chip}">${kind.icon}</span>
          <span class="grow">
            <h5>Option ${i + 1} — ${esc(title)}</h5>
            <p>${esc(desc || '')}</p>
          </span>
        </button>`;
      }).join('')}
    ...
    <div class="body-pad" style="gap:12px">
      <div class="hero-card" style="padding:13px 16px">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
          <p class="lead" style="margin:0;display:flex;align-items:center;gap:8px"><span class="oc-icon-chip" style="--chip:${kind.chip};width:26px;height:26px">${kind.icon}</span>Option ${st.option + 1} — ${esc(kind.name)}</p>
        </div>
